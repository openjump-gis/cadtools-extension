...
public abstract class CircleMiddleStyle extends ExtendedDecorationStyle {
    private final static int DIAMETER = 8;
    private Stroke circleStroke = new BasicStroke(2);

    private CircleMiddleStyle(String name, boolean start, String iconFile,
            boolean filled) {
    ...
    private Shape toShape(Point2D p0, Point2D p1) {
        ...
        return new Ellipse2D.Double(mid.getX() - (DIAMETER / 2d), mid.getY()
                - (DIAMETER / 2d), DIAMETER, DIAMETER);
    }

    public static class Start extends CircleMiddleStyle {
        public Start() {
            super("Geo 2", true, "geo1a.png", true);
        }
    }

    public static class End extends CircleMiddleStyle {
        public End() {
            super("Geo 2", false, "geo1a.png", true);
        }
    }
}
